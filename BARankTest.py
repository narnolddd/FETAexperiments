...
file_grow = "experiments/BARankGenerate.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/BARankLike.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/like.tmp"
results = "experiments/BARank1000results.dat"
...
for ex in range(10):
    for time in times:
        tmp = re.sub("NAME","BARank"+str(time),growdata)
    ...
    for time in times:
        print(time)
        tmplike = re.sub("NAME","BARank"+str(time),likedata)
        curve = np.zeros(100)
...
plt.legend()
plt.title("Likelihoods of different changepoint times BA -> Rank")
plt.xlabel("Time")

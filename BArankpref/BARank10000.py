...
similarities = "experiments/BArankpref/BARank10000Similarities.dat"
...
for ex in range(numExperiments):
    for link in nolinks:
        ...
        for beta in betas:
            ...
            tmp3 = re.sub("NAME","BARank-10000-"+link+"-"+str(beta),fitdata)
            with open(fit_tmp,'w') as f2:
                f2.write(tmp3)
                f2.close()
            os.system("java -jar feta3-1.0.0.jar "+fit_tmp+" > "+dump)

            file = "experiments/BArankpref/BARank-10000-"+link+"-"+str(beta)+".dat"

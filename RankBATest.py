...
file_grow = "experiments/RankBAGenerate.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/RankBALike.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/like.tmp"
results = "experiments/RankBA1000results.dat"
...
for ex in range(10):
    for time in times:
        tmp = re.sub("NAME","RankBA"+str(time),growdata)
    ...
    for time in times:
        print(time)
        tmplike = re.sub("NAME","RankBA"+str(time),likedata)
        curve = np.zeros(100)
...
plt.legend()
plt.title("Likelihoods of different changepoint times Rank -> BA")
plt.xlabel("Time")

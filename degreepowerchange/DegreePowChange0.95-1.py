...
file_grow = "experiments/degreepowerchange/DegreePowGrow0.95-1.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/degreepowerchange/DegreePowLike0.95-1.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/degreepowerchange/like.tmp"
results = "experiments/degreepowerchange/degreepower0.95-1-10000results.dat"
...
for ex in range(10):
    for time in times:
        tmp = re.sub("NAME","DegreePow-0.95-1-"+str(time),growdata)
    ...
    for time in times:
        print(time)
        tmplike = re.sub("NAME","DegreePow-0.95-1-"+str(time),likedata)
        curve = np.zeros(100)

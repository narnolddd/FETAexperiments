...
results = "experiments/degreepowerchange/degreepower1.05-1-10000results.dat"
...
for ex in range(10):
    for time in times:
        tmp = re.sub("NAME","DegreePow-1.05-1-"+str(time),growdata)
    ...
    for time in times:
        print(time)
        tmplike = re.sub("NAME","DegreePow-1.05-1-"+str(time),likedata)
        curve = np.zeros(100)

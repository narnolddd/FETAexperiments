...
file_grow = "experiments/degreepowerchange/DegreePowGrow1-0.9.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/degreepowerchange/DegreePowLike1-0.9.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/degreepowerchange/like1-0.9.tmp"
results = "experiments/degreepowerchange/degreepower1-0.9-10000results.dat"
...
for ex in range(10):
    for time in times:
        tmp = re.sub("NAME","DegreePow-1-0.9-"+str(time),growdata)
    ...
    for time in times:
        print(time)
        tmplike = re.sub("NAME","DegreePow-1-0.9-"+str(time),likedata)
        curve = np.zeros(100)

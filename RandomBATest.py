...
file_grow = "experiments/RandomBAGenerate.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/RandomBALike.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/like.tmp"
results = "experiments/RandBA1000results.dat"
...
for time in times:
    tmp = re.sub("NAME","RandBA"+str(time),growdata)
...
for time in times:
    print(time)
    tmplike = re.sub("NAME","RandBA"+str(time),likedata)
    curve = np.zeros(100)
...
plt.legend()
plt.title("Likelihoods of different changepoint times Random -> BA")
plt.xlabel("Time")

...
file_grow = "experiments/BARandomGenerate.json"
grow_tmp = re.sub(".json","tmp.json",file_grow)
file_like= "experiments/BARandomLike.json"
like_tmp = re.sub(".json","tmp.json",file_like)
dump = "experiments/like.tmp"
results = "experiments/BARand1000results.dat"
...
# for time in times:
#     tmp = re.sub("NAME","BARand"+str(time),growdata)
...
for time in times:
    print(time)
    tmplike = re.sub("NAME","BARand"+str(time),likedata)
    curve = np.zeros(100)
...
plt.legend()
plt.title("Likelihoods of different changepoint times BA -> Random")
plt.xlabel("Time")
